...
# ryomakaraoke One-Line Installer
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        -y|--yes) CONFIRM="n" ;;
        -l|--local) LOCAL="y" ;; # this installs ryomakaraoke from local source
...
if [ -z "$DISPLAY_LIST" ]; then
    INSTALL_LIST="ryomakaraoke (via uv)"
else
    INSTALL_LIST="$DISPLAY_LIST, ryomakaraoke (via uv)"
fi
...
# Install ryomakaraoke
echo "Installing ryomakaraoke via uv..."

if uv tool list | grep -q "ryomakaraoke"; then
    echo "ryomakaraoke is already installed. Upgrading..."
    if [ "$LOCAL" == "y" ]; then
...
if [ $INSTALL_SHORTCUTS -eq 1 ]; then
    ...
    ryomakaraoke_BIN=$(command -v ryomakaraoke || echo "$HOME/.local/bin/ryomakaraoke")
    SHARE_DIR="$HOME/.local/share/ryomakaraoke"
    mkdir -p "$SHARE_DIR"
    ICON_PATH="$SHARE_DIR/logo.icns"
    ICON_URL="https://raw.githubusercontent.com/vicwomg/ryomakaraoke/refs/heads/master/ryomakaraoke/static/icons/logo.icns"
    if [ ! -f "$ICON_PATH" ]; then
    ...
    if [ "$OS_TYPE" == "Darwin" ]; then
        ...
        create_macos_app() {
            ...
            osacompile -o "$app_path" -e "tell application \"Terminal\"
                activate
                do script \"$ryomakaraoke_BIN $args\"
            end tell"
...
EOF
        ...
        create_macos_app "ryomakaraoke" ""
        create_macos_app "ryomakaraoke (headless)" "--headless"
...
EOF
        ...
        if [ -d "$HOME/Desktop" ]; then
            create_linux_desktop "ryomakaraoke" "" "ryomakaraoke.desktop"
            create_linux_desktop "ryomakaraoke (headless)" "--headless" "ryomakaraoke-headless.desktop"
...
echo "Then, simply run: ryomakaraoke"
if [ $INSTALL_SHORTCUTS -eq 1 ]; then
