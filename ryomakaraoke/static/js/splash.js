...
const hasBgVideo = ryomakaraokeConfig.hasBgVideo;
let currentVideoUrl = null;
let hlsInstance = null;
let idleTime = 0;
let screensaverTimeoutSeconds = ryomakaraokeConfig.screensaverTimeout;
let bg_playlist = [];
...
const endSong = async (reason = null, showScore = false) => {
  if (showScore && !ryomakaraokeConfig.disableScore) {
    isScoreShown = true;
...
const playBGMusic = async (play) => {
  ...
  if (play) {
    if (ryomakaraokeConfig.disableBgMusic) return;
    ...
    $(audio).animate({ volume: ryomakaraokeConfig.bgMusicVolume }, 2000);
  } else {
    if (audio) {
...
const playBGVideo = async (play) => {
  ...
  if (play) {
    if (ryomakaraokeConfig.disableBgVideo) return;
    if (!autoplayConfirmed) return;
...
const setupOverlayMenus = () => {
  if (ryomakaraokeConfig.hideOverlay) {
  ...
  const triggerInactivity = () => {
    ...
    cursorVisible = false;
    $("#menu a").fadeOut();
    if (ryomakaraokeConfig.showSplashClock) {
      setTimeout(() => {
...
const handleUnsupportedBrowser = () => {
  if (!isSupportedBrowser) {
    ...
    warningMessage.innerHTML =
      ryomakaraokeConfig.translations.unsupportedBrowser;
...
const toggleBGMedia = (configKey, playFn, disabled) => {
  ryomakaraokeConfig[configKey] = disabled;
...
const PREFERENCE_EFFECTS = {
  ...
  disable_score:       (v) => { ryomakaraokeConfig.disableScore = v; },
  show_splash_clock:   (v) => {
    ryomakaraokeConfig.showSplashClock = v;
    v ? startClock() : (stopClock(), $("#clock").hide());
  },
  hide_overlay:        (v) => {
    ryomakaraokeConfig.hideOverlay = v;
    $("#bottom-container, #top-container").toggle(!v);
  },
  hide_url:            (v) => { $("#qr-code, #screensaver-qr").toggle(!v); },
  bg_music_volume:     (v) => {
    ryomakaraokeConfig.bgMusicVolume = v;
  ...
  screensaver_timeout: (v) => {
    ...
    ryomakaraokeConfig.screensaverTimeout = v;
  },
};
...
const setupSocketEvents = () => {
  ...
  socket.on('connect_error', (error) => {
    ...
    flashNotification(ryomakaraokeConfig.translations.socketConnectionLost, "is-danger");
  });
...
$(function () {
  ...
  setupUIScaling();
  if (ryomakaraokeConfig.showSplashClock) startClock();
  setupScreensaver();
  setupOverlayMenus();
  setupVideoPlayer();
